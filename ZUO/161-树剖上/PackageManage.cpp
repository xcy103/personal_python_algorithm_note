...
using ll  = long long;
int head[MAXN],nxt[MAXN],to[MAXN],cntg;
int fa[MAXN],dep[MAXN],siz[MAXN],son[MAXN],top[MAXN],dfn[MAXN],cntd;
int n,m;
int sum[MAXN<<2],change[MAXN<<2];
bool update[MAXN<<2];
...
int main(){
    ...
    cin.tie(nullptr);
    cin>>n;
    //u为什么从2开始，因为我们设置1(0)为根，题目说了1(0)不依赖任何
    //比如0 0 0 1 1 5,表示从第2(1)个开始
    for(int u=2,v;u<=n;u++){
        cin>>v;
        v++;
        addEdge(v,u);
