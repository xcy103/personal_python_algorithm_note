// 有依赖的背包(模版)
// 物品分为两大类：主件和附件
// 主件的购买没有限制，钱够就可以；附件的购买有限制，该附件所归属的主件先购买，才能购买这个附件
// 例如，若想买打印机或扫描仪这样的附件，必须先购买电脑这个主件
// 以下是一些主件及其附件的展示：
// 电脑：打印机，扫描仪 | 书柜：图书 | 书桌：台灯，文具 | 工作椅：无附件
// 每个主件最多有2个附件，并且附件不会再有附件，主件购买后，怎么去选择归属附件完全随意，钱够就可以
// 所有的物品编号都在1~m之间，每个物品有三个信息：价格v、重要度p、归属q
// 价格就是花费，价格 * 重要度 就是收益，归属就是该商品是依附于哪个编号的主件
// 比如一件商品信息为[300,2,6]，花费300，收益600，该商品是6号主件商品的附件
// 再比如一件商品信息[100,4,0]，花费100，收益400，该商品自身是主件(q==0)
// 给定m件商品的信息，给定总钱数n，返回在不违反购买规则的情况下最大的收益
// 测试链接 : https://www.luogu.com.cn/problem/P1064
// 测试链接 : https://www.nowcoder.com/practice/f9c6f980eeec43ef85be20755ddbeaf4

/*
Author: yangka
Date: 2026-03-18 22:55:01
*/

#include <bits/stdc++.h>
using namespace std;
const int MAXN = 33001;
const int MAXM = 61;
using ll  = long long;

int cost[MAXM],val[MAXM],fans[MAXN],follows[MAXM][2];
bool king[MAXM];
int dp1[MAXM][MAXN],n,m;
int dp2[MAXN];
void clean(){
    for(int i=1;i<=m;i++){
        fans[i] = 0;
    }
}
//没有空间压缩版本
int compute1(){
    memset(dp1,0,sizeof(dp1));
    int p = 0;// 上一个主件编号
    for(int i=1;i<=m;i++){
        if(king[i]){
            int fan1 = (fans[i] >= 1) ? follows[i][0] : -1;
            int fan2 = (fans[i] >= 2) ? follows[i][1] : -1;

            for(int j=0;j<=n;j++){
                //不选当前主商品
                dp1[i][j] = dp1[p][j];
                //只选  主键
                if(j>=cost[i]){
                    dp1[i][j] = max(dp1[i][j],dp1[p][j-cost[i]]+val[i]);
                }
                //选1号粉丝
                if(fan1 != -1 && j >= cost[i] + cost[fan1]){
                    dp1[i][j] = max(dp1[i][j],dp1[p][j-cost[i] - cost[fan1]]+
                val[i]+val[fan1]);
                }
                //选2号
                if(fan2 != -1 && j >= cost[i] + cost[fan2]){
                    dp1[i][j] = max(dp1[i][j],dp1[p][j-cost[i] - cost[fan2]]+
                val[i]+val[fan2]);
                }
                //都选
                if(fan1!=-1 && fan2!=-1 && j >= cost[i] + cost[fan1] + cost[fan2]){
                    dp1[i][j] = max(dp1[i][j],dp1[p][j- cost[i] - cost[fan1] - cost[fan2]]+
                val[i]+val[fan1]+val[fan2]);
                }
            }
        }
        p = i;
    }
    return dp1[p][n];
}
//空间压缩版本
int compute2(){
    memset(dp2,0,sizeof(dp2));
    for(int i=1;i<=m;i++){
        if(king[i]){
            //这里不选可以不用讨论了，因为直接继承了
            int fan1 = (fans[i] >= 1) ? follows[i][0] : -1;
            int fan2 = (fans[i] >= 2) ? follows[i][1] : -1;
            
            for(int j=n;j>=cost[i];j--){
                //只选主
                dp2[j] = max(dp2[j],dp2[j-cost[i]]+val[i]);
                // 2. 主 + 附件1
                if (fan1 != -1 && j >= cost[i] + cost[fan1]) {
                    dp2[j] = max(dp2[j],
                        dp2[j - cost[i] - cost[fan1]] + val[i] + val[fan1]);
                }

                // 3. 主 + 附件2
                if (fan2 != -1 && j >= cost[i] + cost[fan2]) {
                    dp2[j] = max(dp2[j],
                        dp2[j - cost[i] - cost[fan2]] + val[i] + val[fan2]);
                }

                // 4. 主 + 附件1 + 附件2
                if (fan1 != -1 && fan2 != -1 &&
                    j >= cost[i] + cost[fan1] + cost[fan2]) {

                    dp2[j] = max(dp2[j],
                        dp2[j - cost[i] - cost[fan1] - cost[fan2]]
                        + val[i] + val[fan1] + val[fan2]);
                }
            }
            
        }
    }
    return dp2[n];
}
int main(){
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    while(cin>>n>>m){
        clean();
        for(int i=1;i<=m;i++){
            int v,p,q;
            cin >> v >> p >> q;
            cost[i] = v;
            val[i] = v*p;
            king[i] = (q==0);
            if(q!=0){
                follows[q][fans[q]++] = i;
            }
        }
    }
    cout << compute1() << '\n';

    return 0;
}