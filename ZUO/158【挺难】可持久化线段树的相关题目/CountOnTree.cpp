...
//可持久化
int root[MAXN],ls[MAXT],rs[MAXT],siz[MAXT],cntt;
//st
int stjump[MAXN][MAXP],deep[MAXN];
...
int query(int jobk,int l,int r,int u,int v,int lca,int lcafa){
    if(l==r) return l;
    int lsize = siz[ls[u]] + siz[ls[v]] - siz[ls[lca]] - siz[ls[lcafa]];
    int mid = (l + r) / 2;
    if(lsize>=jobk){
...
int main(){
    ...
    dfs(1,0);
    for(int i=1,u,v,k,last = 0;i<=m;i++){
        cin>>u>>v>>k;
        u^=last;
        last = kth(u,v,k);
        cout << last << '\n';
