...
// 1. 重链剖分：按子树大小选重儿子
// 2. 长链剖分：按子树高度选长儿子
// 3. 信息一致：两者维护信息相同，只是划分标准不同
// 4. 查询复杂度：任意点到根为 O(1)
// 5. 预处理复杂度：整体 O(n)，维护与子树高度相关信息
...
ui s;
//建图
int head[MAXN],nxt[MAXN],to[MAXN],cntg = 0;
//长剖
int stjump[MAXN][MAXP],dep[MAXN],len[MAXN],son[MAXN],top[MAXN],dfn[MAXN],cntd = 0;
int high[MAXN],up[MAXN],down[MAXN];
...
void dfs2(int u,int t){
    ...
    for(int e = head[u],v;e;e=nxt[e]){
        v = to[e];
        if(v!=stjump[u][0] && v!=son[u]) dfs2(v,v);
    }
}
void prepare(){
    dfs1(root,0);
